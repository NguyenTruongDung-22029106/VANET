...
def get_config():
    ...
    return SimpleNamespace(
        ...
        objective_profile='qoe_real',
        chunk_playback_sec=2.0,
        rl_step_interval_s=2.0,            # align RL step ~ 1 segment for real-QoE demo
        qoe_default_session_id='default',
        qoe_focus_car_name='car1',         # lock requesting car to telemetry player (set '' to disable)
        qoe_event_timeout_s=0.75,         # wait time for each segment telemetry before fallback/proxy
        qoe_strict_event=False,           # True => phạt thêm nếu thiếu event segment
        qoe_missing_event_penalty=3.0,
        qoe_max_quality_index=3,          # 4 levels: 0..3
        qoe_alpha_startup=0.18,           # P.1203-like startup penalty coefficient
        qoe_alpha_rebuffer_time=0.35,     # P.1203-like rebuffer time penalty coefficient
        qoe_alpha_rebuffer_count=0.20,    # P.1203-like rebuffer count penalty coefficient
        qoe_beta_quality=0.80,            # P.1203-like quality contribution
        qoe_beta_switch=0.25,             # P.1203-like quality-switch penalty
        qoe_w_delay=1.0,                  # proxy QoE term (used when real event is missing)
        qoe_w_stall=1.5,                  # proxy QoE term (used when real event is missing)
        qoe_w_smooth=0.5,                 # proxy QoE term (used when real event is missing)
        reward_clip_min=-25.0,
        reward_clip_max=0.0,
        qea_qoe_calibration_json='results/qea_qoe_calibration.json',

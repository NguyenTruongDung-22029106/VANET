...
class QEAJointCAUA:
    ...
    Objective: minimize calibrated QoE surrogate cost.
    """
    ...
    def __init__(
        ...
        self.chunk_playback_sec = max(float(_cfg(config, 'chunk_playback_sec')), 1e-6)
        self.qoe_w_delay = float(_cfg(config, 'qoe_w_delay'))
        self.qoe_w_stall = float(_cfg(config, 'qoe_w_stall'))
        self.qoe_w_smooth = float(_cfg(config, 'qoe_w_smooth'))
        self.qoe_calib = self._load_qoe_calibration()
    ...
    def _load_qoe_calibration(self):
        """
        Optional calibration coefficients fitted from real runtime QoE logs.
        File format:
          {"intercept": ..., "w_proxy": ..., "w_mu": ..., "w_sigma": ...}
        """
        raw = getattr(self.config, 'qea_qoe_calibration_json', None)
        if not raw:
            return None
        path = str(raw)
        ...
        try:
            ...
            return {
                ...
                'w_proxy': float(data.get('w_proxy', 1.0)),
                'w_mu': float(data.get('w_mu', 0.0)),
                'w_sigma': float(data.get('w_sigma', 0.0)),
            }
        except Exception:
            return None

    def _apply_qoe_calibration(self, proxy_cost, mu_k, sigma_norm):
        if self.qoe_calib is None:
            return float(proxy_cost)
        val = (
            self.qoe_calib['intercept']
            + self.qoe_calib['w_proxy'] * float(proxy_cost)
            + self.qoe_calib['w_mu'] * float(mu_k)
            + self.qoe_calib['w_sigma'] * float(sigma_norm)
        )
        if not np.isfinite(val) or val <= 0.0:
            return float(max(proxy_cost, 0.0))
        return float(val)
    ...
    def _evaluate_individual(self, X_i: np.ndarray, Y_i: np.ndarray) -> float:
        """
        Calibrated QoE surrogate for each user k:
          mu_k    = E[D_k] over p_{f,z}
          sigma_k = std(D_k) over p_{f,z}
          proxy_k = w_delay*log1p(mu_k)
                  + w_stall*max(0, mu_k - T_play)/T_play
                  + w_smooth*(sigma_k/T_play)

          qoe_k = calibration(proxy_k, mu_k, sigma_k/T_play)
        Fitness = Σ_k qoe_k (minimize).
        """
        total = 0.0
        if self.F <= 0 or self.Z <= 0:
            return total

        p_fz = self.p_fz  # shape (F, Z), sum = 1
        t_play = self.chunk_playback_sec
        for l in range(self.L):
            uav = self.uavs[l]
            Y_l = Y_i[l]  # (F, Z)
                    ...
                    for z_req in range(self.Z):
                        ...
                        if int(Y_l[f, z_req]) == 1:
                            ...
                            if z_plus is not None:
                                pair = (z_req, z_plus)
                                if pair not in d_trans_by_pair:
                                    d_trans_by_pair[pair] = _models_cost(
                                        source_node=car, target_node=uav, config=self.config,
                                ...
                                d_cur = d_trans_by_pair[pair]
                            else:
                                d_cur = d_miss_by_z[z_req]

                        mu_k += p * d_cur
                        m2_k += p * (d_cur ** 2)

                sigma_k = math.sqrt(max(m2_k - (mu_k ** 2), 0.0))
                sigma_norm = sigma_k / t_play
                proxy_cost = (
                    self.qoe_w_delay * math.log1p(max(mu_k, 0.0))
                    + self.qoe_w_stall * max(0.0, (mu_k - t_play) / t_play)
                    + self.qoe_w_smooth * sigma_norm
                )
                total += self._apply_qoe_calibration(proxy_cost, mu_k, sigma_norm)

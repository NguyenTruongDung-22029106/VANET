...
class SdnVanetRyuApp(app_manager.RyuApp):
    ...
    def _init_csv(self):
        ...
        with open(self._csv_path, 'w') as f:
            f.write(
                ...
                'bitrate,z_cached,cache,delay,delay_term,stall_norm,smooth_norm,'
                'qoe_source,qoe_session_id,segment_idx,qoe_mos,qoe_cost,startup_sec,rebuffer_sec,'
                'rebuffer_count,quality_index,switch_magnitude,segment_download_sec,buffer_sec,'
                'distance_2d,overshoot,oor_penalty,base_reward,reward_final\n'
            )
    ...
    def _write_csv(
        ...
        bitrate, z_cached, cache, delay, delay_term, stall_norm, smooth_norm,
        qoe_source, qoe_session_id, segment_idx, qoe_mos, qoe_cost, startup_sec,
        rebuffer_sec, rebuffer_count, quality_index, switch_magnitude,
        segment_download_sec, buffer_sec, distance_2d, overshoot, oor_penalty,
        ...
        try:
            with open(self._csv_path, 'a') as f:
                ...
                f.write(
                    ...
                    f'{bitrate},{z_cached},{cache},{delay:.6f},{float(delay_term):.6f},'
                    f'{float(stall_norm):.6f},{float(smooth_norm):.6f},{qoe_source},{qoe_session_id},'
                    f'{int(segment_idx)},{float(qoe_mos):.6f},{float(qoe_cost):.6f},'
                    f'{float(startup_sec):.6f},{float(rebuffer_sec):.6f},{float(rebuffer_count):.6f},'
                    f'{float(quality_index):.6f},{float(switch_magnitude):.6f},'
                    f'{float(segment_download_sec):.6f},{float(buffer_sec):.6f},{float(distance_2d):.6f},'
    ...
    def _control_loop(self):
        ...
        if training:
            ...
            if max_steps <= 0:
                max_steps = None
        step_interval_s = float(getattr(self._config, 'rl_step_interval_s', 0.10))
        ...
        if max_steps is None:
            self.logger.info(
                "*** REST control loop running... (unbounded), step_interval=%.2fs ***",
                step_interval_s,
            )
        else:
            self.logger.info(
                "*** REST control loop running... max_steps=%d, step_interval=%.2fs ***",
                max_steps, step_interval_s,
            )
        ...
        while not self._control_stop.is_set() and (max_steps is None or step < max_steps):
            try:
                ...
                delay_term = _to_float(info.get("delay_term", 0.0), 0.0)
                stall_norm = _to_float(info.get("stall_norm", 0.0), 0.0)
                smooth_norm = _to_float(info.get("smooth_norm", 0.0), 0.0)
                qoe_source = str(info.get("qoe_source", "unknown"))
                qoe_session_id = str(info.get("qoe_session_id", ""))
                segment_idx = int(_to_float(info.get("segment_idx", 0), 0.0))
                qoe_mos = _to_float(info.get("qoe_mos", 0.0), 0.0)
                qoe_cost = _to_float(info.get("qoe_cost", -reward), -reward)
                startup_sec = _to_float(info.get("startup_sec", 0.0), 0.0)
                rebuffer_sec = _to_float(info.get("rebuffer_sec", 0.0), 0.0)
                rebuffer_count = _to_float(info.get("rebuffer_count", 0.0), 0.0)
                quality_index = _to_float(info.get("quality_index", 0.0), 0.0)
                switch_magnitude = _to_float(info.get("switch_magnitude", 0.0), 0.0)
                segment_download_sec = _to_float(info.get("segment_download_sec", delay), delay)
                buffer_sec = _to_float(info.get("buffer_sec", 0.0), 0.0)

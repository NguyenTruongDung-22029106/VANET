#!/usr/bin/env python3
"""
Reference: Simple VANET topology with roads and RSUs as AccessPoints.
- roads=N: Mininet-WiFi generates N roads; cars move along these roads (not random on blank canvas).
- RSUs are AccessPoints connected to a switch; cars associate to nearest RSU by distance, or use ad-hoc if out of range.
Use this as reference for topology in main_thesis.py (e.g. roads, propagation, car-AP association).
"""
...
def topology():
    "Create a network with roads and RSUs."
    ...
    # roads=8: roads are drawn; cars move along them
    ...
    # Car–RSU association by distance: if car within range of AP -> managed + IP; else ad-hoc
    while True:
        for car in net.cars:
            test = 0
            for ap in net.aps:
